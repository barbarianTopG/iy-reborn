...
local success, hui = pcall(gethui or get_hidden_ui or get_hui or get_hidden_gui or gethiddenui or gethiddengui)
...
everyClipboard = missing("function", setclipboard or toclipboard or set_clipboard or writeclipboard or (Clipboard and Clipboard.set))
...
PARENT = nil;
local Main = Instance.new("ScreenGui", (hui or COREGUI or game:GetService("CoreGui")))
...
Title.TextSize = 18;
Title.Text = "Infinite Yield Reborn"
do
	local emoji = ({
		["01 01"] = "🎆",
		[(function(Year)
...
CMDs[#CMDs + 1] = {
	NAME = 'fakeout',
	DESC = 'Tp to the void and then back (useful to kill people attached to you)'
}
CMDs[#CMDs + 1] = {
	NAME = 'antivoid',
...
function ESP(plr, logic)
	task.spawn(function()
		for i, v in pairs((hui or COREGUI):GetChildren()) do
...
addcmd('dupetools', {
	...
	local Tools, TempPos = {}, Vector3.new(math.random(-32, 32), 32, math.random(-32, 32))
	for i = 1, LOOP_NUM do
...
addcmd('tpunanchored', {
	...
	for i, v in pairs(players) do
		...
		for _, part in pairs(workspace:GetDescendants()) do
			if Players[v].Character:FindFirstChild('Head') and part:IsA("BasePart") and part.Anchored == false and not part:IsDescendantOf(speaker.Character) and part.Name == "Torso" == false and part.Name == "Head" == false and part.Name == "Right Arm" == false and part.Name == "Left Arm" == false and part.Name == "Right Leg" == false and part.Name == "Left Leg" == false and part.Name == "HumanoidRootPart" == false then
...
addcmd("addallplugins", {
	...
	for _, filePath in ipairs(listfiles("")) do
		...
		if fileName and fileName:lower() ~= "iy_fe.iy" and not isfolder(fileName) and not table.find(PluginsTable, fileName) then
			addPlugin(fileName)
		end
	end
end)
